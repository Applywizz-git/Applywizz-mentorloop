  ...
  try {
    ...
    try {
      ...
      if (clientUid && mentorUid) {
        // Insert the notification for the mentor about the new booking request
       // Correctly inserting the mentor notification
          await supabase.from("notifications").insert({
            user_id: clientUid,                  // Who created the notification (client)
            recipient_id: mentorUid,             // Mentor receives the notification
            kind: "booking.action_required",     // Notification type
            title: "New booking request",        // Notification title
            body: "A new booking request needs your confirmation.", // Notification body
            message: "A new booking request needs your confirmation.",
            payload: { booking_id: bookingId, slot_id: selectedSlot.id, mentor_id: mentor.id },
