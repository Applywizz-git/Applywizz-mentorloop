...
// const MENTOR_DASHBOARD_URL = "https://mentorship-loop-1.vercel.app/dashboard/mentor";
...
//       const passwordLink = `https://mentorship.vercel.app/set-password?mentorId=${mentorId}`;
...
//         const confirmUrl =  `${MENTOR_DASHBOARD_URL}`;
...
const CLIENT_ID = Deno.env.get("CLIENT_ID")!;
const CLIENT_SECRET = Deno.env.get("CLIENT_SECRET")!;
const SENDER_EMAIL = Deno.env.get("SENDER_EMAIL")!; // e.g., support@applywizz.com (organizer mailbox)
...
/**
 * ✅ NEW: Create a Teams-enabled calendar event via Microsoft Graph.
 * - Organizer is your service mailbox (SENDER_EMAIL).
 * - Adds mentor + client as attendees.
 * - Sets isOnlineMeeting=true so Graph generates a Teams join link.
 * - Outlook automatically sends calendar invites to all attendees.
 * - Returns eventId, joinUrl, and the Outlook webLink for deep-linking.
 */
async function createTeamsCalendarEvent(params: {
  organizerEmail: string;   // use SENDER_EMAIL
  subject: string;
  startIsoUtc: string;      // e.g., "2025-10-21T13:00:00Z"
  endIsoUtc: string;        // e.g., "2025-10-21T13:30:00Z"
  attendeeEmails: string[]; // mentor + client
  timezone?: string;        // "UTC" or "India Standard Time"
}) {
  const {
    organizerEmail,
    subject,
  ...
  const token = await getGraphToken();
  const endpoint = `https://graph.microsoft.com/v1.0/users/${encodeURIComponent(organizerEmail)}/events`;

  // NOTE:
  // - We strip trailing "Z" because we pass timeZone explicitly.
  // - Prefer header ensures response normalizes to the zone we expect.
  const body = {
    subject,
    body: {
      contentType: "HTML",
      content: "<p>Your mentorship session has been scheduled.</p>",
    },
    start: { dateTime: startIsoUtc.replace("Z", ""), timeZone: timezone },
    end:   { dateTime: endIsoUtc.replace("Z", ""),   timeZone: timezone },
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
      Prefer: 'outlook.timezone="UTC"', // change to IST if you prefer Graph to localize invite body
      // Optional extra safety against duplicate events on retries:
      // "Outlook-Idempotency-Key": crypto.randomUUID?.() ?? `${Date.now()}-${Math.random()}`
    },
  ...
  if (!res.ok) {
    ...
    console.error("[Graph Calendar] create event error:", err);
    throw new Error("Failed to create Teams calendar event");
  }

  const json = await res.json();
  return {
    eventId: json.id as string,
    joinUrl: json.onlineMeeting?.joinUrl ?? null, // Teams join URL
    webLink: json.webLink ?? null,                // Outlook calendar web link
  };
}
...
// ✅ Create a Teams calendar event using Microsoft Graph
async function createTeamsEvent(
  subject: string,
  startIso: string,
  endIso: string,
  attendees: string[]
  ...
  const token = await getGraphToken();
  const endpoint = `https://graph.microsoft.com/v1.0/users/${encodeURIComponent(SENDER_EMAIL)}/events`;
  ...
  const json = await res.json();
  if (!res.ok) {
    console.error("Graph Event Error:", json);
    throw new Error(`Failed to create Teams event: ${JSON.stringify(json)}`);
  }

  return json.onlineMeeting?.joinUrl ?? "";
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (mode === "book") {
      ...
      // To mentor (if we found a contact email) — build clickable HTML link with bookingId
      if (mentorEmail) {
        const confirmUrl = MENTOR_DASHBOARD_URL; // dashboard handles accept/reject with the booking id context
...
if (mode === "confirm") {
  ...
  // If direct call, update status to 'confirmed'
  if (!isDbWebhookPayload(body)) {
    const { error } = await sb
      .from("bookings")
      .update({ status: "confirmed" })
  ...
  // ✅ 1. Load booking + slot times to get meeting start & end
  ...
  const menteeEmail = booking.mentee_email;
  const startIso = new Date(booking.time_slots.start_iso).toISOString();
  const endIso = new Date(booking.time_slots.end_iso).toISOString();

  // ✅ 2. Resolve mentor email
  const { mentor, mentorEmail } = await getMentorContact(booking.mentor_id);
  if (!mentorEmail || !menteeEmail) {
    console.warn("[booking-mails] Missing mentor or mentee email; skipping Teams event creation.");
    await sendGraphMail(
      menteeEmail || "",
      "Session Confirmed",
      "Your session has been confirmed by your mentor.",
    ...
    return new Response(JSON.stringify({ ok: true, warning: "missing email(s)" }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }

  // ✅ 3. Create Teams calendar event through Graph API
  const subject = `Mentorship Session${mentor?.name ? " with " + mentor.name : ""}`;
  const joinUrl = await createTeamsEvent(
    subject,
    startIso,
    endIso,
  ...
  // ✅ 4. Save Teams URL in the bookings table
  const { error: upErr } = await sb
    ...
    .eq("id", bookingId);
  if (upErr) {
    console.warn("[booking-mails] warning: failed to store Teams info:", upErr.message);
  }

  // ✅ 5. Optionally send branded confirmation emails (Graph already sent invites)
  await sendGraphMail(
    menteeEmail,
    "Your mentorship session has been scheduled",
    `Your session is confirmed. You can join here: ${joinUrl}`,
    "Text"
  );
  await sendGraphMail(
    mentorEmail,
  ...
  return new Response(JSON.stringify({ ok: true, joinUrl }), {
    headers: { ...corsHeaders, "Content-Type": "application/json" },
  });
}
