...
const SUPABASE_URL = Deno.env.get("SUPABASE_URL")!;   // e.g. https://your-site.vercel.app
const SERVICE_ROLE_KEY = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
const WEBHOOK_SECRET = Deno.env.get("WEBHOOK_SECRET") || "";

// Supabase admin client
const FRONTEND_URL = "https://mentorship-loop-1.vercel.app";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // // 3) Generate confirmation/access link (use invite, fallback to recovery) → redirect to frontend
let linkData, linkErr;

// Try INVITE first
({ data: linkData, error: linkErr } = await sb.auth.admin.generateLink({
  type: "invite",
  email,
  options: { redirectTo: `${FRONTEND_URL}/login` }, // ✅ use FRONTEND_URL
}));

// If user already exists, fall back to RECOVERY
if (linkErr) {
  ({ data: linkData, error: linkErr } = await sb.auth.admin.generateLink({
    type: "recovery",
    email,
...
if (linkErr || !linkData?.properties?.action_link) {
  throw new Error(`Could not generate invite/recovery link: ${linkErr?.message || "no link"}`);
}
